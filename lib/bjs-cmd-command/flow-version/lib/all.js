	...
	function inner(path) {
		...
		ll.forEach(function(f) {
			...
			if (stat.isDirectory()) {
				...
				tmp = PATH.relative(options.mbase, file).replace(/\\|\\\\/g, '/');
				if (/\.js$/.test(file)) {
...
function disposeSelf(src, options) {
	...
	var pattern = /src=["'][^\s"']+\.js\?*[0-9A-Za-z]*["']*|href=["'][^\s"']+\.css\?*[0-9A-Za-z]*["']*|src=["']*[^\s"']+\.(png|jpg|gif|bmp)\?*[0-9A-Za-z]*["']*|url\([^\s"'\)]+\.(png|jpg|gif|bmp)\?*[0-9A-Za-z]*\)/g;
	...
	if (matches && matches.length > 0) {
		...
		matches.forEach(function(item) {
			...
			// 如果是./和../的相对路径，需要跟当前文件所在文件夹合成绝对路径
			if (/^\./.test(tmp)) {
				tmp = PATH.join(PATH.dirname(src), tmp).replace(/\\|\\\\/g, '/');
			} else if (/^\//.test(tmp)) {
				tmp = PATH.join(process.cwd(), '../' + tmp).replace(/\\|\\\\/g, '/');
			} else {
