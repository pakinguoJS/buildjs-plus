...
module.exports = function(fileObj, options) {
	...
	if (!options.mbase) {
		console.log('[Error]: '.red + 'options.mbase is required!');
		return;
	}
	...
	function inner(file) {
		...
		if (matches && matches.length > 0) {
			matches.forEach(function(item) {
				...
				/@/.test(ext) ? ext = ext.substring(0, ext.indexOf('@')) : null;
...
function disposeSelf(src, options){
	...
	if (matches && matches.length > 0) {
			matches.forEach(function(item) {
				...
				// console.log(PATH.dirname(rpath) + '/' + PATH.basename(tmp, ext) + (version !== '' ? '@v' + version : '') + ext)
			})
		}
