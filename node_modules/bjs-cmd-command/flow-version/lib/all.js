...
function newVersion(fileObj, options) {
	...
	if (!options.sbase) {
		process.stdout.write('[Error]: '.red + 'options.sbase is required!');
		return;
	}
 ...
 *
 * @description
 * 当前文件的更新会影响到其他文件对其的引用，以当前文件路径（模块路径）为正则规则，做项目遍历更新
 */
function disposeText(src, pattern, options, version, type) {
	...
	function inner(file) {
		// 由于存在被clear功能删除了旧版本文件的情况，文件还没处理的时候就已经被删除，故需要过滤
		...
		if (matches && matches.length > 0) {
			...
			matches.forEach(function(item) {
				...
				/@v/.test(ext) ? ext = ext.substring(0, ext.indexOf('@v')) : null;
				...
				// 如果是/开头，则是需要跟base合成绝对路径
				if (/^\//.test(rpath)) {
					tmp = PATH.join(options.sbase, rpath).replace(/\\|\\\\/g, '/');
				}

				// 模式匹配，校验变更的文件xx/xx.js与遍历的文件yy/xx[@v..].js是否一致，如果不是，则返回
				tmp = tmp.replace(/@v[a-zA-Z0-9]+\./, '.');
				if (src.indexOf(tmp) < 0) {
					return;
				}
		...
		// 针对seajs模块id——define(...)
		// var _pattern = /define(\s)*\(["'][^\s"']+["'](\s)*,/g;
		// var replacePattern = /['",;]|define(\s)*\(/g;
		// matches = content.match(_pattern);
		// if (matches && matches.length > 0) {
		// 	matches = unique(matches);
		// 	matches.forEach(function(item) {
		// 		var rpath = item.replace(replacePattern, '');
		// 		var tmp = PATH.join(options.mbase, rpath).replace(/\\|\\\\/g, '/');
		// 		var idx = tmp.indexOf('@v');

		// 		// tmp需要提取到不带版本号
		// 		tmp = idx > -1 ? tmp.substring(0, idx) + '.js' : tmp + '.js';
		...
		// 		var v = FS.statSync(tmp).mtime.getTime() / 1000;
		// 		if (idx > -1) {
		// 			// 如果版本号一致，则没必要回写，直接返回
		// 			var currentVersion = rpath.match(/@v[a-zA-Z0-9]+/g);
		// 			if (currentVersion && currentVersion[0].indexOf(v) > -1) {
		// 				console.log('SAME:' + item, v)
		// 				return;
		// 			}
		// 		}

		// 		console.log(item, v)
		// 		content = content.replace(new RegExp(rpath.replace(/\.|\//g, '\\$&'), 'g'), rpath.substring(0, rpath.indexOf('@v') > 0 ? rpath.indexOf('@v') : rpath.length) + '@v' + FS.statSync(tmp).mtime.getTime() / 1000);
		// 		mark = true;
		// 	})
		// }
		...
		if (mark) {
			...
			if (file !== src) {
				...
				// clear version
				require('./clear.js')(PATH.dirname(file));
			}
		}
	}
}
...
function disposeSelf(src, options) {
	...
	var pattern = /src=["'][^\s"']+\.js["']*|['"][^\s"']+\.js['"]|href=["'][^\s"']+\.css["']*|['"][^\s"']+\.css['"]|src=["']*[^\s"']+\.(png|jpg|gif|bmp)["']*|url\([^\s"']+\.(png|jpg|gif|bmp)\)/g;
	var replacePattern = /href=|src=|['",;]|url\(|\)|define(\s)*\(/g;
