...
bjs.run = function() {
	...
	switch (argvs[2]) {
		...
		case 'wi':
			...
			require(FS.readFileSync(PATH.join(__dirname, 'config.conf'), 'utf8')).watchInotify(bjs.config);
			break;
		case 'cg':
			bjs.change(argvs[3]);
			break;
		case 'release':
			...
			require(FS.readFileSync(PATH.join(__dirname, 'config.conf'), 'utf8')).buildjsWithVersion(bjs.config);
			break;
		case 'test':
			require(FS.readFileSync(PATH.join(__dirname, 'config.conf'), 'utf8')).test();
			break;
	}
}
