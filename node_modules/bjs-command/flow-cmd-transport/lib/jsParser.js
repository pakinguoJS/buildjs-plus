...
function jsParser(fileObj, options) {
    ...
    try {
        ...
        console.log('[Error]: js parse error'.red + fileObj.src);
        console.log((e.message + ' [ line:' + e.line + ', col:' + e.col + ', pos:' + e.pos + ' ]').red);
    ...
    if (!meta) {
        ...
        console.log('[Warn]: found non cmd module '.yellow + '"' + fileObj.src + '"');
        return;
    }
    ...
    if (meta.dependencyNode) {
        ...
        // grunt.log.verbose.writeln(deps.length ? 'found dependencies ' + deps : 'found no dependencies');
    }
    ...
    // success
    console.log('[Succ]: transported '.green + '"' + fileObj.src + '"');
}
...
function parseDependencies(fpath, options) {
    ...
    function relativeDependencies(fpath, options, basefile) {
        ...
        if (!grunt.file.exists(fpath)) {
            if (!/\{\w+\}/.test(fpath)) {
                console.log('[Warn]: can\'t find '.yellow + fpath);
