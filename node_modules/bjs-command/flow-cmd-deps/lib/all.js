...
module.exports = function(fileObj, options) {
	...
	var files = processDeps(fileObj.src[0], options);
...
function processDeps(src, options) {
	...
	var mapFile = PATH.join(options.dest, MAPJSON);
	FS.existsSync(mapFile) ? null : grunt.file.write(mapFile, '{}');

	// read exsited mapped source
	var json = require(mapFile);
	...
	for (itm in json) {
		m in json[itm]['_'] ? changes.push(itm) : null;
	}

	grunt.file.write(mapFile, JSONNICE(json));
